...
for asdf = 1:10
    ...
    for a = 1:N
        ...
        distances(a) = norm(vectors(a,:));
    end
    for a = 2:N
        angles(a) = dot(vectors(a-1,:),vectors(a,:))/(distances(a-1)*distances(a));
    end
    angles(1) = dot(vectors(N,:),vectors(2,:))/(norm(vectors(N,:))*norm(vectors(2,:)));
    for a = 1:N-1
        seperation(a) = abs(angles(a+1)-angles(a));
    end
    seperation(N) = abs(angles(1) - angles(N));
    for a = 1:N
        if seperation(a) > closeness || abs(angles(a)) > pi/3
            if a == 1
                if seperation(N) > closeness 
                    if DEBUGSPIKE
                        figure(bonemaskFigure);
                        plot(nodes(a,1),nodes(a,2),'g+','MarkerSize',5);
                    end
                     nodes(1,:) = nodes(N,:);
                     nodes(N+1,:) = nodes(1,:);
